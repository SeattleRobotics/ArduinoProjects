...
#define PIN_SOUND_DIGITAL 10
#define PIN_SOUND_LED 9
...
#define PANEL_SOUND_ANALOG 4
#define PANEL_SOUND_DIGITAL 5
int panels[] = {PANEL_DASHBOARD, PANEL_TEMPERATURE, PANEL_HUMIDITY, PANEL_INFRARED, PANEL_SOUND_ANALOG, PANEL_SOUND_DIGITAL};
...
void loop() {
  ...
  switch (currentPanel) {
    ...
    case PANEL_SOUND_ANALOG:
      processSoundAnalog();
      break;

    case PANEL_SOUND_DIGITAL:
      processSoundDigital();
      break;
...
void processSoundAnalog() {

  int sensorValue = analogRead(PIN_SOUND_ANALOG);
  Serial.println (sensorValue, DEC);

  drawPanel("Analog Sound", String(sensorValue));
  delay (250);
}

void processSoundDigital() {

  int sensorValue = digitalRead(PIN_SOUND_DIGITAL);
  Serial.println (sensorValue, DEC);

  if (sensorValue == 1) {
    digitalWrite(PIN_SOUND_LED, HIGH);
  }
  else {
    digitalWrite(PIN_SOUND_LED, LOW);
  }

  drawPanel("Digital Sound", String(sensorValue));
  delay (250);
}
...
void drawPanel(String line1, String line2) {
